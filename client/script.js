...
const repeatMyParam = () => {
   axios.get(`http://localhost:3000/repeat/a-string`)
   .then(res => {
    return res.data
   }) 
 //YOUR CODE HERE
...
const  getCustomQuery = () => {
    axios.get(`http://localhost:3000/?a-query-of-your-choice-on-the-end!`)
}
...
const ohMy = () => {
    ...
    .then(res => {
     for(let i = 0; i < res.data; i++) {
       const newElement = document.createElement('p').textContent = res.data[i]
...
const  getCustomQuery = () => {
    axios.get(`http://localhost:3000/?a-query-of-your-choice-on-the-end!?second-query`)
}
